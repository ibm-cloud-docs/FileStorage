...
- **Data-At-Rest Encryption** [(Available in select data centers.)](/docs/FileStorage?topic=FileStorage-selectDC)
   - Provider-managed encryption for data-at-rest is provided at no additional cost.
- **All Flash Backed Storage** [(Available in select data centers.)](/docs/FileStorage?topic=FileStorage-selectDC)
   - All flash storage for volumes can be provisioned at 2 IOPS/GB or higher levels.
- **Snapshots** [(Available in select data centers.)](/docs/FileStorage?topic=FileStorage-selectDC).
   - Captures point-in-time data snapshots non-disruptively.
- **Replication**  [(Available in select data centers.)](/docs/FileStorage?topic=FileStorage-selectDC)
   - Available when storage is provisioned in [select data centers](/docs/FileStorage?topic=FileStorage-selectDC).
...
- **10 IOPS per GB** is designed for the most demanding workloads such as those created by NoSQL databases, and data processing for Analytics. This tier is available for storage that is provisioned up to 4 TB in [select data centers](/docs/FileStorage?topic=FileStorage-selectDC) only.
...
You can select hourly or monthly billing for a File volume. The type of billing that is selected for a volume applies to its snapshot space and replicas. For example, if you provision a volume with hourly billing, any snapshots or replica fees are billed hourly. If you provision a volume with monthly billing, any snapshots or replica fees are billed monthly.

 * With **hourly billing**, the number of hours that the File volume existed on the account is calculated at the time the volume is deleted or at the end of the billing cycle, which ever comes first. Hourly billing is a good choice for storage that is used for a few days or less than a full month. Hourly billing is available for storage that is provisioned in [select data centers](/docs/FileStorage?topic=FileStorage-selectDC) only.
