...
3. Mode de facturation. Si vous avez sélectionné un centre de données avec des fonctionnalités améliorées (indiqué par un astérisque), vous avez le choix entre une facturation mensuelle ou horaire.
     ...
     {:note}
4. Entrez votre taille de stockage dans la zone **Taille**.
5. Sélectionnez **Endurance (IOPS hiérarchisées)**.
6. Sélectionnez le niveau d'IOPS requis par votre application.
    ...
    - L'option **10 IOPS par Go** est adaptée aux charges de travail les plus intensives, telles que celles créées par les bases de données NoSQL et le traitement de données pour Analytics. Ce niveau est disponible dans des [centres de données sélectionnés](/docs/infrastructure/FileStorage?topic=FileStorage-selectDC) pour un stockage qui est mis à disposition à hauteur de 4 To.
7. Cliquez dans la zone se trouvant sous **Espace d'image instantanée** puis sélectionnez la taille de l'image instantanée dans la liste. Cet espace vient en complément de votre espace utilisable. Pour les considérations et recommandations relatives à l'espace d'instantané, lisez la section [Commande d'instantanés](/docs/infrastructure/FileStorage?topic=FileStorage-ordering-snapshots).
...
3. Mode de facturation. Si vous avez sélectionné un centre de données avec des fonctionnalités améliorées (indiqué par un astérisque), vous avez le choix entre une facturation mensuelle ou horaire.
     ...
     {:note}
4. Entrez votre taille de stockage dans la zone **Taille**.
5. Sélectionnez **Performance (IOPS allouées)**.
