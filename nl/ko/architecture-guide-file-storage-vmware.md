...
다음 단계를 수행하여 {{site.data.keyword.BluSoftlayer_full}}의 vSphere 5.5 및 vSphere 6.0 환경에서 {{site.data.keyword.filestorage_full}}를 주문하고 구성할 수 있습니다. VMWare 호스트에 대한 연결이 8개 넘게 필요한 경우 NFS {{site.data.keyword.filestorage_short}}를 사용하는 방법이 최선입니다.
...
{{site.data.keyword.filestorage_short}} 오퍼링은 NFS 연결을 통해 액세스되고 마운트됩니다. VMware 배치에서는 공유 스토리지로서 최대 64개의 ESXi 호스트로 단일 볼륨을 마운트할 수 있습니다. 또는 다수의 볼륨을 마운트하여 vSphere Storage DRS(Distributed Resource Scheduling)를 사용하기 위한 스토리지 클러스터를 작성할 수 있습니다.
...
- 크기를 결정할 때는 필요한 워크로드와 처리량의 크기를 고려하십시오. 용량(IOPS/GB)에 비례하여 선형적으로 성능을 스케일링하는 Endurance 서비스에서는 크기 문제가 중요합니다. 이와 반대로 Performance 서비스에서는 관리자가 용량과 성능을 별도로 선택할 수 있습니다. Performance 서비스에서는 처리량 요구사항 문제가 중요합니다. <br/> **참고**: 처리량 계산은 IOPS x 16KB입니다. IOPS는 50/50 읽기/쓰기 혼합의 16KB 블록 크기를 기반으로 측정됩니다. <br/> **참고**: 블록 크기를 늘리면 처리량은 증가되지만 IOPS는 감소됩니다. 예를 들어, 32KB 블록으로 블록 크기를 두 배로 늘리면 최대 처리량은 그대로 유지되지만 IOPS는 반으로 감소됩니다.
- NFS는 다수의 추가 파일 제어 오퍼레이션(일부 예: `lookup`, `getattr` 및 `readdir`)을 사용하여 활용합니다. 읽기/쓰기 오퍼레이션에 추가하여 이러한 오퍼레이션은 IOPS로 계수될 수 있으며, 이는 오퍼레이션 유형 및 NFS 버전에 따라 다양할 수 있습니다.
...
- NFS v3 및 NFS v4.1 모두는 {{site.data.keyword.BluSoftlayer_full}} 환경에서 지원됩니다. 하지만 {{site.data.keyword.IBM}}에서는 NFS v3 사용을 권장합니다. NFS v4.1이 Stateful 프로토콜이므로(NFSv3과 같은 Stateless가 아님), 프로토콜 문제가 네트워크 이벤트 중에 발생할 수 있습니다. NFS v4.1은 모든 오퍼레이션을 거부한 후에 잠금 교정을 수행해야 합니다. 이러한 오퍼레이션이 실행되는 동안 장애가 발생할 수 있습니다.

#### NFS 프로토콜 VMware 기능 지원 매트릭스.
<table>
  <caption>표 1에서는 vSphere 기능이 두 개의 서로 다른 NFS 버전에 적용될 때 이 기능을 표시합니다.</caption>
 <thead>
  <tr>
   <th>vSphere 기능</th>
   <th>NFS 버전 3</th>
...
스냅샷을 사용하려면 “스냅샷 영역”이 필요하다는 점을 유념하십시오. 영역은 **조치**를 클릭하고 **스냅샷 영역 추가**를 선택하여 **볼륨 세부사항** 페이지를 통해 초기 프로비저닝 후에 또는 초기 볼륨 주문 시에 확보될 수 있습니다.
...
Endurance {{site.data.keyword.filestorage_short}} 볼륨을 복원하려면 Endurance {{site.data.keyword.filestorage_short}}에 상주하는 모든 VM의 전원을 꺼야 합니다. 프로세스 중에 데이터 손상을 방지하기 위해 ESXi 호스트의 볼륨을 임시로 마운트 해제해야 합니다.
...
런던의 볼륨이 복구된 후에는 암스테르담 볼륨의 스냅샷을 가져옵니다. 그런 다음 런던으로 장애 복구하고 런던의 컴퓨팅 인스턴스의 기본 볼륨으로 다시 연결할 수 있습니다. 
...
**참고**: 손상되거나 해킹되거나 감염되었는지 여부와 무관하게 올바르지 않은 데이터는 스냅샷 스케줄 및 스냅샷 보유에 따라 복제됩니다. 최소 복제 창을 사용하면 보다 나은 복구 지점 목표가 제공됩니다. 하지만 이에 따라 올바르지 않은 데이터의 복제에 대해 반응하는 시간도 단축될 수 있습니다.
...
4. 위치를 선택하십시오. 개선된 기능의 데이터 센터는 별표로 표기됩니다. 이전에 주문된 ESXi 호스트와 동일한 위치에 새 스토리지가 추가되는지 확인하십시오.
...
VMware 및 Jumbo 프레임에 대한 자세한 정보는 [여기](https://kb.vmware.com/s/article/1003712){:new_window}에서 찾을 수 있습니다.
...
1. 정적 라우트를 구성하려면 Performance 또는 Endurance를 활용 중인 각 ESXi 호스트로 SSH를 실행하고 다음 명령을 실행하십시오. 참고로, 결과 IP 주소 Ping 명령(첫 번째 명령)을 가져와서 이를 아래에 표시된 esxcli 네트워크 명령과 함께 사용해야 합니다.
...
2. 참고로, 정적 라우트는 ESXi 5.0 이전에는 재부팅 간에 지속성이 없습니다. 추가된 정적 라우트의 지속성을 보장하려면 이러한 명령을 /etc/rc.local.d/ 디렉토리에 있는 각 호스트의 local.sh 파일에 추가해야 합니다. 이를 수행하려면 Visual Editor를 사용하여 local.sh 파일을 열고 exit 0 행 위에서 실행되도록 위의 명령을 추가해야 합니다.
   ...
   - VMware KB 기사에 대한 링크는 다음과 같습니다. [ESXi 호스트에서 VMkernel 포트에 대한 정적 라우트 구성](https://kb.vmware.com/s/article/2001426){:new_window}.
...
Jumbo 프레임은 소스 디바이스 <-> 스위치 <-> 라우터 <-> 스위치 <-> 대상 디바이스의 전체 네트워크 경로에서 동일하게 구성되어야 합니다. 전체 체인이 동일하게 설정되지 않은 경우, 기본값은 체인의 최하위 설정입니다. {{site.data.keyword.BluSoftlayer_full}}의 네트워크 디바이스는 현재 9,000으로 설정되어 있습니다. 모든 고객 디바이스는 동일하게 설정되어야 합니다.
...
5. **고급** 탭을 선택하십시오.
6. **Jumbo 프레임**을 선택하고 NIC에 따라 "사용 안함" 값을 원하는 값으로 변경하십시오(예: 9kB MTU 또는 9,014바이트).
7. 모든 대화 상자에 대해 **확인**을 클릭하십시오.

참고로, 변경 시에는 몇 초 동안 NIC의 네트워크 연결이 끊깁니다. 또한 재부팅을 실행하여 변경사항을 적용해야 합니다.
