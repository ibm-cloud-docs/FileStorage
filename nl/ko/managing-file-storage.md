...
**참고** - 권한 부여 및 연결은 스토리지와 동일한 데이터 센터에 있는 호스트에 대해 수행할 수 있습니다. 그러나 여러 개의 계정이 있는 경우에는 한 계정의 호스트에 다른 계정의 스토리지에 액세스하도록 권한을 부여할 수 없습니다. 
...
**참고** - 호스트 측 문제를 방지하려면, 누락된 드라이브나 데이터 손상을 방지할 수 있도록 액세스 권한을 취소하기 전에 운영 체제에서 해당 스토리지 볼륨을 마운트 해제하십시오.
...
**참고** - 특정 호스트에서 여러 볼륨에 대한 연결을 끊으려면 각 볼륨에 대해 액세스 권한 취소 조치를 반복해야 합니다.
...
**참고** - 특정 볼륨에서 여러 호스트에 대한 연결을 끊으려면 각 호스트에 대해 액세스 권한 취소 조치를 반복해야 합니다.
...
3. 즉시 볼륨을 취소하는지 또는 볼륨이 프로비저닝된 기념일에 취소하는지를 확인하십시오.
   >**참고** - 주기에 볼륨을 취소하는 옵션을 선택하는 경우에는 해당 주기 이전의 취소 요청을 무효화할 수 있습니다.
4. **계속** 또는 **닫기**를 클릭하십시오. 
