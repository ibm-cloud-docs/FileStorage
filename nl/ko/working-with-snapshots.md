...
스냅샷 스케줄은 시간별, 일별, 주별 간격으로 설정될 수 있으며 각각에는 별도의 보유 주기가 있습니다. 각 스토리지 볼륨에는 최대 50개의 스케줄된 스냅샷(시간별, 일별, 주별 스케줄의 혼합일 수 있음)과 수동 스냅샷이 있을 수 있습니다. 
...
**참고**: 스냅샷 영역은 늘리는 것만 가능합니다. 줄일 수는 없습니다. 실제 필요한 영역의 양이 판별될 때까지는 보다 작은 양의 영역을 선택할 수 있습니다. 자동화된 스냅샷과 수동 스냅샷은 영역을 공유한다는 점을 기억하십시오. 
...
**주의**: 복제 기능을 사용 중인 경우에는 삭제하려는 스케줄이 복제에서 사용하는 스케줄이 아닌지 확인하십시오. 복제 스케줄 삭제에 대한 자세한 정보를 보려면 [여기](replication.html)를 클릭하십시오.
...
4. 사용할 스냅샷 옆에 있는 **조치**를 클릭하고 **복원**을 클릭하십시오. <br/>
   >**참고**: 복원을 완료하면 스냅샷이 작성된 후 작성되거나 수정된 데이터가 손실됩니다. 이 데이터 손실은 스토리지 볼륨이 스냅샷 작성 시와 동일한 상태로 돌아가기 때문에 발생합니다.  
...
6. 스토리지 볼륨을 호스트에 마운트하고 다시 연결하십시오.
  - Linux의 {{site.data.keyword.filestorage_short}} 지시사항을 보려면 [여기](accessing-file-storage-linux.html)를 클릭하십시오.
    >**참고** - 볼륨을 복원하면 복원된 스냅샷 이전에 작성된 모든 스냅샷이 삭제됩니다. 
