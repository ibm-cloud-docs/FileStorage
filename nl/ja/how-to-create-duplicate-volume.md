...
>**注** - {{site.data.keyword.containerlong}} の専用アカウント・ユーザーの場合、[{{site.data.keyword.containerlong_notm}} 資料](/docs/containers/cs_storage_file.html#backup_restore)でボリュームを複製するためのオプションをご確認ください。
...
6. 新規ボリュームのサイズを更新して元のものより大きくすることができます。 デフォルトでは、元のボリュームのサイズが設定されます。
    - **注**: {{site.data.keyword.filestorage_short}} は、ボリュームの元のサイズの 10 倍にサイズ変更可能です。
