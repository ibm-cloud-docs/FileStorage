...
ストレージが削除されると、そのボリューム上のデータを指すポインターがすべて除去されるため、データはアクセス不能になります。 物理ストレージが別のアカウントに再プロビジョンされると、新しいポインター・セットが割り当てられます。 物理ストレージ上に存在していたデータに新しいアカウントでアクセスする方法はありません。新しいポインター・セットはすべて 0 を示します。新しいデータにより、その物理ストレージ上に存在していたアクセス不能データはすべて上書きされます。
