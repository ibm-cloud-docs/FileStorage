...
スナップショット・スケジュールを使用して、ストレージ・ボリュームのポイント・イン・タイム・リファレンスを作成する頻度と時間を決定します。 ストレージ・ボリュームごとに最大 50 個のスナップショットを作成できます。 スケジュールは、[{{site.data.keyword.cloud}} コンソール](https://{DomainName}/){: external}の**「ストレージ」** > **「{{site.data.keyword.filestorage_short}}」**タブで管理します。
...
1. ストレージ・ボリュームをクリックして、**「アクション」**をクリックし、**「スナップショット・スペースの変更」**をクリックします。
...
ポータル内の手動で削除されなかった手動スナップショットは、スペースの制限に達すると自動的に削除されます。最も古いスナップショットが最初に削除されます。
{:note}
