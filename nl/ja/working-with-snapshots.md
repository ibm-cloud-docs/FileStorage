...
スナップショット・スケジュールを作成することで、ストレージ・ボリュームのポイント・イン・タイム・リファレンスを作成する頻度と時間を決定できます。 ストレージ・ボリュームごとに最大 50 個のスナップショットを作成できます。 スケジュールは、[{{site.data.keyword.slportal}} ![外部リンク・アイコン](../../icons/launch-glyph.svg "外部リンク・アイコン")](https://control.softlayer.com/){:new_window}の**「ストレージ」**>**「{{site.data.keyword.filestorage_short}}」**タブで管理します。
