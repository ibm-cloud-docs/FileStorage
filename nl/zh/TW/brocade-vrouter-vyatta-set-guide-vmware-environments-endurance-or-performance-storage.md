...
您可以在使用 {{site.data.keyword.filestorage_full}} 的 VMware 環境內，配置 Brocade vRouter (Vyatta) 應用裝置的高可用性 (HA) 配置。請將下列資訊與[進階單一網站 VMware 參照架構](https://console.bluemix.net/docs/infrastructure/virtualization/advanced-single-site-vmware-reference-architecturesoftlayer.html){:new_window}一起使用，以在 VMware 環境中設定其中一個儲存空間選項。
