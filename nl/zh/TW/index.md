...
在 {{site.data.keyword.BluSoftlayer_full}} 環境中，同時支援 NFS 第 3 版及 NFS 4.1 版。不過，偏好使用 NFS 第 3 版，因為 NFS 4.1 版是有狀態的通訊協定（不像 NFSv3 是無狀態的通訊協定），而且在網路事件期間可能會發生通訊協定問題。NFS 4.1 版必須靜止所有作業，然後完成鎖定收回。在相當忙碌的 NFS 檔案伺服器上，延遲增加可能會造成中斷。缺少 NFS 4.1 版多路徑/幹線也可能延長 NFS 作業回復。

## 提交訂單

當您準備好提交訂單時，請遵循[這裡](provisioning-file-storage.html)的指示。若要使用 VMware 佈建檔案儲存空間，請按一下[這裡](architecture-guide-file-storage-vmware.html)
...
- [在 CentOS 中裝載 NFS/檔案儲存空間](mounting-nsf-file-storage.html)
