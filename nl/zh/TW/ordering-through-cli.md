...
您可以使用 SLCLI 來訂購通常是透過 [{{site.data.keyword.cloud}} 型錄](https://{DomainName}/catalog){: external}來訂購的產品。在 SL API 中，一張訂單可能是由多重訂單容器所組成。訂單 CLI 只能用於一個訂單容器。
...
如需透過 API 來訂購 {{site.data.keyword.filestorage_short}} 的相關資訊，請參閱 [order_file_volume](https://softlayer-python.readthedocs.io/en/latest/api/managers/file/#SoftLayer.managers.file.FileStorageManager.order_file_volume){: external}。若要能夠存取所有新增特性，請訂購 `Storage-as-a-Service Package 759`。
{:tip}
...
如需同時授權限制的相關資訊，請參閱[常見問題](/docs/infrastructure/FileStorage?topic=file-storage-faqs)。
{:important}
