...
{{site.data.keyword.BluSoftlayer_full}} 将推出新版的 {{site.data.keyword.blockstorageshort}} 和 {{site.data.keyword.filestorage_full}}！新的存储器在精选数据中心内可用，是更高 IOPS 级别的闪存支持的存储器，具有针对静态数据的磁盘级别加密。对于在精选数据中心内供应的所有存储器，都将自动供应新版的 {{site.data.keyword.blockstorageshort}} 和 {{site.data.keyword.filestorage_short}}。
