...
IOPS is measured based on a load profile of 16 KB blocks with random 50 percent reads and 50 percent writes. Workloads that differ from this profile might experience lower performance.

## What happens when I use a smaller block size for measuring performance?

Maximum IOPS can be obtained even if you use smaller block sizes. However, the throughput will be lower. For example, a volume with 6000 IOPS would have the following throughput at various block sizes:
...
There's no need for pre-warming. You’ll observe the specified throughput immediately upon provisioning the volume.
...
## When provisioning Performance or Endurance {{site.data.keyword.filestorage_short}}, is the allocated IOPS enforced by instance or by volume?
